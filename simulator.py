...
class Simulator:
    ...
    #     self.add_ring(ring_position, [0., np.pi / 2., 0.], 2.5, 0.5, 'big-ring.urdf')
    ...
    #     angles = np.linspace(0, 2 * np.pi, num_rings, endpoint=False)  # Close loop without duplicating first point
    ...
    #         start = points[i]
    #         end = points[(i + 1) % len(points)]  # Wrap around using modulo for cyclic behavior
    ...
    def place_rings(self):
        ...
        for i in range(num_rings):
            ...
            ring_position = np.array([x_position, 0., hover_height])  # y is 0 since it's a straight line along x-axis
        ...
        for i in range(num_rings - 1):
            y_position = start_y + i  # Calculate the y position (to the left) for each ring
            ring_position = np.array([5, y_position, hover_height])  # x is 5 since we start from the end of the previous route

            # Place a ring at this position
            self.add_ring(ring_position, [0., 0., -np.pi/2], 2.5, 0.5, 'big-ring.urdf') 
    ...
    # ADD RING BUT DISABLE COLLISION DETECTION (NEEDED FOR SPECIAL ROUTE ER)
    def add_ring(self, pos, ori, radius, width, urdf):
        if len(ori) == 3:
            # ori is rpy
            q = self.bullet_client.getQuaternionFromEuler(ori)
    ...
    def check_ring(self, drone):
        position, orientation = self.bullet_client.getBasePositionAndOrientation(drone['id'])
        ...
        if self.is_inside_ring(self.rings[drone['cur_ring']], position):
            ...
            if drone['cur_ring'] == len(self.rings) - 1:
                if self.t < self.max_time_steps:
                    ...
                    return True
            drone['cur_ring'] += 1  # Increment the current ring if not the final or max time reached
        return False
